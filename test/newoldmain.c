...
*/
void	find_wall_v(t_map *set, float ray, int *x, int *y)
{
	int	cx;
	int	cy;
	int	px;
	...
	cy = set->case_y;
	if (set->diry < 0)
		py = cy * set->wall_size - 1;
	else
		py = cy * set->wall_size + set->wall_size;
	px= set->player_x + (set->player_y - py) * tan(fabs(ray) * M_PI / 180);
	cx = px / set->wall_size;
	cy = py / set->wall_size;
	if (tan(fabs(ray) * M_PI / 180) == 0.0)
		add = 0;
	else
		add = (float)set->wall_size * tan(fabs(ray) * M_PI / 180);
	printf("truc deux h = x = %d, y = %d\n", px, py);
...
void	find_wall_h(t_map *set, float ray, int *x, int *y)
{
	...
	py = set->player_y + (set->player_x - px) * tan(fabs(ray) * M_PI / 180);
	cx = px / set->wall_size;
	cy = py / set->wall_size;
	if (tan(fabs(ray) * M_PI / 180) == 0.0)
		add = 0;
