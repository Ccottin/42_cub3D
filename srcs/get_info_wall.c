...
/*   Updated: 2022/08/04 22:06:35 by ccottin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_info_wall_0(t_data *data, char *str, int i, char c)
{
	...
	if (c == 'N')
	{
		data->north_texture = fill_info_wall(str, i, y);
		if (!data->north_texture)
			return (-1);
	}
	if (c == 'S')
	{
		data->south_texture = fill_info_wall(str, i, y);
...
int	get_info_wall_1(t_data *data, char *str, int i, char c)
{
	...
	if (c == 'E')
	{
		data->east_texture = fill_info_wall(str, i, y);
		if (!data->east_texture)
			return (-1);
	}
	if (c == 'W')
	{
		data->west_texture = fill_info_wall(str, i, y);
